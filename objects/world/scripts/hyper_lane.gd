...
func cut_player(length: float) -> void:
	...
	if not _player.autopilot:
		AudioClipManager.play("res://sounds/sfx/Bluezone_BC0303_futuristic_user_interface_transition_006.mp3", 0.15)
...
func civilize(civilization: Civilization, star: Star) -> void:
	...
	if from.civilization == to.civilization:
		...
		for point_index: int in range(points.size() - 1):
			var p1_3d: Vector3 = points[point_index] + _player.global_position
			var p2_3d: Vector3 = points[point_index + 1] + _player.global_position
